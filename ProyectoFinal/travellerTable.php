    ...
    <?php 
        ...
        if(isset($_GET['aksi']) == 'delete'){
            ...
            if(mysqli_num_rows($cek) == 0){
                ...
                if($delete){
                    header("Location: travellerTable.php");die();
                }
                else{
        ...
        <div class="content">
            ...
            <?php 
                while($row = mysqli_fetch_array($result)){
                    echo" 
                    <tr> 
                        <td>".$row['Dni_viajero']."</td> 
                        <td>".$row['Nombre']."</td> 
                        <td>".$row['Direccion']."</td> 
                        <td>".$row['Telefono']."</td> 
                        <td> <a class='btn btn-sm btn-primary' href='travellerEdit.php?nik=".$row['Dni_viajero']."'>Editar datos</a></td>
                        <td> <a class='btn btn-sm btn-danger' href='travellerTable.php?aksi=delete&nik=".$row['Dni_viajero']."' title='Eliminar' onclick='return confirm(\"Estas seguro de borrar los datos de ".$row['Nombre']."?\")'>Eliminar</a></td>
                    </tr>";
                }
            ?>
